...
function AddProject({ projects, saveProjects }) {
  ...
  const handleSubmit = async (event) => { event.preventDefault(); const project = { ...form, id: form.id || `${form.title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '')}-${Date.now()}`, image: form.image || imageLibrary.nexora, screenshots: form.screenshots.filter(Boolean), updatedAt: 'Just now', longDescription: form.description }; const didSave = await saveProjects(form.id ? projects.map((item) => item.id === form.id ? project : item) : [project, ...projects]); if (!didSave) { setSaveError('Could not save this project in browser storage.'); return; } setSaveError(''); setSaved(true); setTimeout(() => navigate('/admin/projects'), 450); };
